...
import { assets } from '../assets/assets';
import { useAdmin } from '../context/adimContext';
import { useDoctors } from '../context/DoctorContext'; // ✅ import doctor context
import { toast } from 'react-toastify';
import { FaChevronDown } from 'react-icons/fa';
...
const Navbar = () => {
    ...
    const location = useLocation();
    const { axios, user, setUser } = useAdmin();
    // const { doctor, authLoading } = useDoctors(); // ✅ get doctor state
    const navigate = useNavigate();
  const { doctor, authLoading } = useDoctors();
    
    console.log('Navbar - authLoading:', authLoading);
    console.log('Navbar - doctor:', doctor);
    const toggleMenu = () => setIsMenuOpen(!isMenuOpen);
    const toggleDropdown = () => setIsDropdownOpen(prev => !prev);
    const closeDropdown = () => setIsDropdownOpen(false);
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center h-16 xlg:mx-14">
                    ...
                    <div className="hidden lg:flex items-center space-x-10">
                        ...
                        {/* ✅ Conditional Doctor link */}
                        {!authLoading && (
                            doctor ? (
                                <Link
                                    to="/doctor-dashboard"
                                    onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
                                    className="px-4 py-1.5 rounded-full text-sm font-medium bg-primary text-white hover:bg-blue-700 transition-colors duration-200"
                                >
                                    Doctor Dashboard
                                </Link>
                            ) : (
